import { Clock, Gauge, Music, Volume2, X } from "lucide-react";
...
interface SettingsModalProps {
	isOpen: boolean;
	onClose: () => void;
	vietnameseVoices: UnifiedVoice[];
	allVoices?: UnifiedVoice[]; // Optional: all voices for debugging
				...
				>
					...
					<div className="p-6 space-y-8">
									...
									<button
										...
										className={`flex-1 px-4 py-2 rounded-lg border transition-colors duration-200 ${
											engine === "native"
												...
												: "bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer"
										}`}
									>
										Native (Offline)
									</button>
								...
								{engine === "edge" && (
									<div className="mt-2 space-y-1">
										...
										<p className="text-xs text-amber-600 dark:text-amber-400">
											⚠️ Lưu ý: Edge TTS có thể không hoạt động trên một số trình
											duyệt do giới hạn CORS/WebSocket. Nếu gặp lỗi, vui lòng
											chuyển sang Native TTS.
										</p>
									</div>
								)}
							</div>
						...
						<div>
							...
							>
								{(showAllVoices && allVoices
									? allVoices
									: vietnameseVoices
								).map((voice) => {
									const voiceKey = voice.shortName || voice.name;
									const voiceLabel = voice.locale
									...
									<div className="space-y-4 pl-8">
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													handleTimerValueChange(parseInt(e.target.value))
												}
												className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-slate-900 dark:accent-slate-100"
											/>
											<div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mt-1">
